...
function SkillPill({ name, desc }: { name: string; desc: string }) {
  return (
    <div className="relative group/pill shrink-0 mx-2">
      <motion.div
      ...
      >
        <span
          className="w-1.5 h-1.5 rounded-full inline-block mr-2 mb-0.5"
      ...
      <div
        ...
        style={{ background: 'rgba(8,8,8,0.95)', backdropFilter: 'blur(12px)', color: 'var(--text-secondary)' }}
      >
        {desc}
        <div
...
function SkillRow({ category, rowIndex }: { category: typeof SKILLS[0]; rowIndex: number }) {
  ...
  return (
    ...
    >
      <div
        className="shrink-0 w-28 text-right pr-4"
      ...
      <div className="flex-1 overflow-hidden relative">
        <div
          ...
          style={{ background: 'linear-gradient(to right, #090d0a, transparent)' }}
        />
        <div
          className="absolute right-0 top-0 bottom-0 w-12 z-10 pointer-events-none"
          style={{ background: 'linear-gradient(to left, #090d0a, transparent)' }}
        />
        <div
...
export default function Skills() {
  ...
  return (
    <section
      ...
      style={{ background: '#090d0a', position: 'relative', zIndex: 1 }}
    >
