  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="text-xs font-mono tracking-wider block mb-1" style={{ color: 'var(--accent)', opacity: 0.55 }}>
            {entry.date}
          </span>
          <h3 className="font-heading font-bold text-lg mb-0.5" style={{ color: 'var(--text-primary)' }}>
            {entry.role}
          </h3>
          <p className="text-sm font-medium mb-3" style={{ color: 'var(--accent)' }}>{entry.org}</p>
          <ul className="space-y-2 text-sm text-left">
            {entry.points.map((pt, i) => (
              <li key={i} className={`flex items-start gap-2 ${isLeft ? 'md:flex-row-reverse md:text-right' : ''} text-left`}>
                <span className="w-1.5 h-1.5 rounded-full mt-1.5 shrink-0" style={{ background: 'var(--accent)', opacity: 0.5 }} />
                <span className="text-left" style={{ color: 'var(--text-secondary)' }}>{pt}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="hidden md:flex flex-col items-center pt-5">
        <motion.div
          ...
          style={{
            ...
            boxShadow: inView ? '0 0 20px rgba(91,127,255,0.8), 0 0 40px rgba(91,127,255,0.3)' : 'none',
          }}
        >
          {inView && (
...
export default function Experience() {
  ...
  return (
    <section ref={sectionRef} id="experience" className="py-28 px-6 md:px-12 lg:px-24 relative overflow-hidden">
      <ScanLine />
      ...
      <motion.div style={{ scale, opacity }}>
        ...
        <div ref={containerRef} className="max-w-5xl mx-auto relative z-10">
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-px" style={{ background: 'rgba(255,255,255,0.08)' }} />
          <div
            ref={lineRef}
            className="hidden md:block absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-px origin-top"
            style={{ background: 'linear-gradient(to bottom, var(--accent), rgba(91,127,255,0.15))' }}
          />
